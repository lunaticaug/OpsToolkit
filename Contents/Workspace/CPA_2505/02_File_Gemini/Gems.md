...
너의 최상위 목표는 **'내용의 완전한 복원'과 '논리적 구조화'**이다. 다른 설명 없이, 최종 결과물은 오직 지정된 형식의 코드 블록으로만 제공해야 한다.
...
3.  **Content Restoration**:
    ...
    * 표에 딸린 각주(`*`)는 표의 일부가 아니다. 표 바로 다음 줄에 일반 텍스트로 배치한다.
    * 표의 '합계' 행이나, 셀 병합 효과를 내는 중요한 헤더 행은 `**bold**` 서식을 적용하여 의미를 강조한다.
    * '답안양식에 따라 제시하시오'라는 문맥이 나타나면, 사용자가 채울 수 있는 빈칸 표를 생성한다.
